...
The system SHALL NOT create, amend, cancel, activate, expire, or verify mandates or eDDA authorizations in this change.

#### Scenario: Existing authorization reference is provided
- **GIVEN** a client submits a supported collection request
- **WHEN** the request includes a mandate reference or direct debit authorization reference
- **THEN** the system SHALL treat the reference as simulator input evidence of an existing authorization
- **AND** the system SHALL NOT create a mandate resource

#### Scenario: Mandate lifecycle request is outside scope
